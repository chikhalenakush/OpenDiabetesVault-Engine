...
public class OverThresholdFilterTest extends Assert {
    ...
    public OverThresholdFilterTest() {
    }
    ...
    public void testFilter() throws ParseException {
        ...
        OverThresholdFilter instance = new OverThresholdFilter(VaultEntryType.BASAL_PROFILE, 1.00, FilterType.BASAL_AVAILABLE, FilterType.BASAL_TH);
        FilterResult result = instance.filter(data);

        for (VaultEntry entry : result.filteredData) {

            assertTrue((entry.getType().toString().equals("BASAL_PROFILE") && entry.getValue() >= 1.0));
